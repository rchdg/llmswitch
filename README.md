...
主供应商返回 429/5xx、连接失败或超时时，Bridge 会按备用链自动切换重试同一请求；切换发生在流式响应开始前，客户端无感（SSE 流中会插入 `: failover: 主 → 备` 注释帧便于排查）。每个备用供应商按自己的默认模型转发（请求中的模型 ID 自动改写）、按自己配置的协议（chat / completions）请求对应端点，密钥沿用各自配置。

备用供应商可以再配置自己的备用链：Bridge 会按层序展开（主的直接备用优先，之后才是这些备用的备用），自动去重、跳过成环（如备用的备用指回主），展开后的候选总数上限 6 个。
